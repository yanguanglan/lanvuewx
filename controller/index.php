...
if(Request::get('state')=='qqreg'){
	...
	$keysArr = array(
			...
			"client_secret" =>'15da5cb01e3036763eba0497f7d04fbd',
			"code" => $accode
	);
...
if(strpos(strtolower($_SERVER ['HTTP_HOST']),'houtai.szdywx.com') !== false || strpos(strtolower($_SERVER ['HTTP_HOST']), 'wx.dxcx.com') !== false){
	Redirect::to('/agencyAdmin/login.html');
}
if(strpos(strtolower($_SERVER ['HTTP_HOST']),'www.weiqimobile.com') !== false){
	$gkw = "微企 、微信营销、微房产、微生活、微餐饮、微汽车、微网站、微商城、微喜帖、微营销、微信定制开发";
	$gde = "微企 , 微信公众智能服务平台,十大微体系:微生活、微菜单、微网站、微会员、微活动、微商城、微服务、微房产、微汽车、微支付、微客服,企业微营销必备。";
	$gti = "微企  - 微信公众智能服务平台 - 微企 ,微信营销,如此简单！";
}else{
	$gkw = "小微、微信营销、微信代运营、微信托管、微网站、微商城、微营销、微信定制开发";
	$gde = "微企 ,微信公众智能服务平台,微企十大微体系:微菜单、微官网、微会员、微活动、微商城、微推送、微服务、微统计、微支付、微客服,企业微营销必备。";
	$gti = "微企 - 微信公众智能服务平台";
}
...
$mu = new Model('user');
		...
		if(!empty($u->un) && !empty($u->pwd) && ($mu->is_real(array('un','pwd')) || $mu->is_real(array('email','pwd')) || $mu->is_real(array('telephone','pwd')))){			
			if($mu->isstop != '0'){
				...
				if(!$_SERVER['IS_OEM'] && (strpos(strtolower($_SERVER ['HTTP_HOST']),'115.29.229.166') === false || strpos(strtolower($_SERVER ['HTTP_HOST']),'ww.com') === false || strpos(strtolower($_SERVER ['HTTP_HOST']),'wx.com') === false)){
	...
	if($uuid==md5(Conf::$management_center_password.$uid)){
		$mu = new Model('user');
		$mu->find($uid);
		if(!$_SERVER['IS_OEM'] && (strpos(strtolower($_SERVER ['HTTP_HOST']),'www.weiqimobile.com') === false && strpos(strtolower($_SERVER ['HTTP_HOST']),'ww.com') === false && strpos(strtolower($_SERVER ['HTTP_HOST']),'wx.com') === false)){
			Redirect::to('/index.html?uid='.$mu->id.'&upwd='.md5($mu->pwd));
		}
		set_user_login($mu);
	}
