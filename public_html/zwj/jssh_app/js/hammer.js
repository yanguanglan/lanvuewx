...
function pv_trans_get(o,t){
	var ystrans = o.style['webkitTransform'].split(')');
...
function pv_trans_set(o,t,v,tm){
	...
	if(tm){
		o.style['webkitTransitionDuration'] = tm+'ms';
		setTimeout(function(){
			o.style['webkitTransitionDuration'] = '0ms';
		},tm);
	}
	o.style['webkitTransform'] = ntrm;
	
}
function pv_transscrollY(jo,js,ld){
	var scy = -ld*jo.data('rellistdatabl');
	js[0].style['webkitTransform'] = 'translate(0px,'+scy+ 'px) translateZ(0)';
}
function pv_transmoveY(o,y){
	o.style['webkitTransitionDuration'] ='0ms';
	o.style['webkitTransform'] = 'translate(0px,'+y+ 'px) translateZ(0)';
}
function pv_transmoveX(o,x){
	o.style['webkitTransitionDuration'] ='0ms';
	o.style['webkitTransform'] = 'translate('+x+'px,0px) translateZ(0)';
}
function pv_transmoveXY(o,x,y){
	o.style['webkitTransitionDuration'] ='0ms';
	o.style['webkitTransform'] = 'translate('+x+'px,'+y+ 'px) translateZ(0)';
}
function pv_transmovetimeY(o,y,t,cb){
	window.istaping = true;
	o.style['webkitTransitionDuration'] = t+'ms';
	o.style['webkitTransform'] = 'translate(0px,'+y+ 'px) translateZ(0)';
	setTimeout(function(){
		o.style['webkitTransitionDuration'] = '0ms';
...
function pv_transmonmentumX(o,v){
	...
	if(xcd<$(o).data('relswipedatamin')){
		...
		o.style['webkitTransform'] = 'translate('+xcd+'px,0px) translateZ(0)';
		$(o).data('relswipedata',xcd);
...
function pv_transmonmentumY(o,v,thescroll){
	...
	if(xcd<$(o).data('rellistdatamin')){
		...
		o.style['webkitTransform'] = 'translate(0px,'+xcd+ 'px) translateZ(0)';
...
(function($){
    $.fn.extend({
        ...
        qscroll: function(m) {
            ...
            this.each(function(){
            	var mainjobj = $(this);
            	if(mainjobj.attr('qqscrolling')=='qq'){
            		return;
            	}else{
            		mainjobj.attr('qqscrolling','qq');
            	}
    			...
    			mainjobj.qmove().on('dragupstart dragdownstart',function(e){
					...
					mbody.data('rellistdata',mbody.data('rellistdatamax'));
					mainjobj.data('winloading',false);
    			});
            });
            return this;
        },
        qflow: function(m) {
            ...
            this.each(function(){
    			...
    			mainjobj.qmove().on('touch',function(e){
    				mbody[0].style['webkitTransitionDuration'] = '0ms';
    				e.preventDefault();
    				return false;
