...
<div class="main" id="wrap">
    	...
    	<div class="p_mod o_address">
    		...
    		<ul>
    		    ...
    		    <?php if(($tpay == 1) OR ($apay == 1)): ?><li><strong>支付方式：</strong>
    		       <?php if($tpay == 1): ?><input type="radio"  name="pay" value="1" checked="checked" style="width:30px" /><img src="http://weixinguanjia.cn/admin2/Extend/Vendor/Tenpay/image/cft.gif" width="135px" height="45px" ><br/><?php endif; ?>
    		       <?php if($apay == 1): ?><input type="radio" name="pay" value="2" style="width:30px;margin-left:65px;"  /><img src="http://weixinguanjia.cn/admin2/Extend/Vendor/Alipay/images/alipay.gif" ></li><?php endif; endif; ?>
    		 </ul>
            </div>
    	</div>
    	<div class="o_btn clearfix">
    		<input type="button" class="o_btn_back ts1" onclick="history.go(-1);" value="返回" />
    		<?php if($tpay == 1): ?><input type="submit" class="o_btn_submit ts1" value="保存并付款" />
    		<?php elseif($apay == 1): ?>
    		<input type="submit" class="o_btn_submit ts1" value="保存并付款" />
    		<?php else: ?>
    		<input type="submit" class="o_btn_submit ts1" value="保存" /><?php endif; ?>
    	</div>
...
function showTip(tipTxt) {
	...
	function tgSubmit(){
		...
		if ($.trim(userPhone) == "") {
         ...
         var patrn = /^(1(([0-9][0-9])|(47)|[8][0126789]))\d{8}$/;
