...
class ProductAction extends BaseAction{
	...
	public function __construct(){
		...
		$this->token		= $this->_get('token')==''? $this->_get('wid'):$this->_get('token');
		//$this->token		= $this->_get('token');
		$this->assign('token',$this->token);
		$this->wecha_id	    = $this->_get('wecha_id') ==''? $this->_get('wxid') : $this->_get('wecha_id');
	...
	public function orderCart(){
		...
		//查询商家支付配置信息
		$payset_model=M('Pay_set');
		$where=array('token'=>$this->_get('token'));
		$paySet = $payset_model->where($where)->find();
		$tpay = 0;
		$apay = 0;
		if($paySet['tpartner']!='' && $paySet['tkey']!='')
		{
			$tpay = 1;
			$payarray['tpartner'] = $paySet['tpartner'];
			$payarray['tkey']     = $paySet['tkey'];
		}
		if($paySet['apartner']!='' && $paySet['akey']!='' && $paySet['aseller_email']!='')
		{
			$apay = 1;
			$payarray['apartner'] = $paySet['apartner'];
			$payarray['akey']     = $paySet['akey'];
			$payarray['aseller_email'] = $paySet['aseller_email'];
		}
		session('payarray',$payarray);

		$this->assign('tpay',$tpay);
		$this->assign('apay',$apay);
		if (IS_POST){
			$allPrice = 0;
			$row=array();
			$carts=$this->_getCart();
			...
			foreach ($carts as $k=>$c){
				...
				$allPrice = $all+$c['price']*$c['count']; //计算总金额
				$pname    = $product_model->where(array('id'=>$k))->find();
				array_push($productesname,$pname['name']);
			}
			$productesnamestr = implode("+",$productesname);
			//把团购、普通购物和餐饮分开
			$normalCart=array();
			$grouponCart=array();
			$diningCart=array();
			...
			if($allPrice>0)
			{
				...
				if($this->_post('pay')==1)
				{
					...
					$reqHandler = new PayAction();
					$reqHandler ->dotenpay($data);
				}
				elseif($this->_post('pay')==2)
				{
					/*$data = array(
					 'ordsubject'   =>$productesnamestr,
					 'ordtotal_fee' =>$allPrice,
					 'ordbody'      =>'订单付款',
					 'token'        =>$this->token,
					 'wecha_id'     =>$this->wecha_id,
					 'trade_no'     =>$orderid
					 );
					 //$reqHandler = new AlipayAction();*/
					$data = array(
					  'WIDsubject'   =>$productesnamestr,
					  'WIDtotal_fee' =>$allPrice,
				      'token'        =>$this->token,
				      'wecha_id'     =>$this->wecha_id,
				      'WIDout_trade_no'=>$orderid
					);
					$reqHandler = new AlipaywapAction();
					$reqHandler ->doalipay($data);
				}
				//$this->assign('reqHandler',$reqHandler);
				//$this->display('Product:paycheck');
				else
				{
