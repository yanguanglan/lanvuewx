...
class ProductAction extends UserAction{
	...
	public function pay()
	{
		if(IS_POST){
			if($this->_post('token')==session('token'))
			{
				...
				if($rs)
				{
					if($data->create()){
						if($data->where($where)->save($_POST)){
							$this->success('修改成功',U('Product/pay',array('token'=>session('token'),'pay'=>1)));
						}else{
							$this->error('操作失败');
						}
					}else{
				...
				else
				{
					$rs=$data->add($_POST);
					if($rs){
						$this->success('添加成功',U('Product/pay',array('token'=>session('token'),'pay'=>1)));
					}else{
						$this->error('操作失败');
					}
				}
		...
		else
		{
			...
			$paySet = $data->where($where)->find();
			$this->assign('paySet',$paySet);
			$this->assign('token',session('token'));
			$this->assign('uid',session('uid'));
			$this->display('pay_set');
		}
	}
